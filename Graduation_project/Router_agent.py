...
SESSION_ID = os.getenv("ERP_SESSION_ID", "demo-session") # demo session cuz setting numbers is unecesarry 
USER_ID = os.getenv("ERP_USER_ID", "demo-user")

def get_db_connection():
    return sqlite3.connect(DB_PATH)

# Ensure tables exist for orchestrator logs/approvals
...
# Governance part and Logging helpers
...
# Specialized Tools (keep @tool for the react loop, it's not necessary otherwise, regular python function can do)
...
def classify_and_route(user_request: str) -> str:
    ...
    #  2: Fallback to keyword scoring if llm classification failes ( hich isnt likely)
    ...
    if gov_result["needs_approval"]:
        with get_db_connection() as conn:
            ...
            conn.commit()
        return f"This request is flagged as {gov_result['risk_level']} risk and requires approval.\nReasons: {', '.join(gov_result['reasons'])}"

    # 4: now route to the correct agent 
    try:
